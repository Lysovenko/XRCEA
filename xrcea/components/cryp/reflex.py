...
def calc_bg(sig_x, sig_y, deg, bf=3.0):
    tbg = np.polyval(np.polyfit(sig_x, sig_y, deg), sig_x)
    sigma2 = ((sig_y - tbg) ** 2).sum() / (len(tbg) - 1.0 - deg)
    sigma2p = None
    coeffs = None
...
def refl_sects(s_x, stripped_y, sigma2, bf=3.0):
    sigma = sigma2**0.5 * bf
    sector = []
    prev = []
    for x, y in zip(s_x, stripped_y):
        if y > sigma:
            ...
            if y < 0.0:
                ...
                sector = []
                prev.append((x, y))
        elif prev and prev[-1][1] - y > y or y <= 0.0:
            prev = [(x, y)]
        else:
            prev.append((x, y))
    if sector:
...
class ReflexDedect:
    ...
    def __init__(self, sector, lambda21=None, i2=0.5):
    ...
    def calc_deviat3(self, x):
        ...
        if the_x[:, 1].min() <= 0.0 or the_x[:, 1].max() > self.max_h:
            return np.inf
        the_x[:, 2] = x[-1]
    ...
    def find_bells_pp(self, sh_type, poss, fposs):
        ...
        opt_x = opt_x.flatten()
        opt_x, sig2 = fmin(
            self.calc_deviat, opt_x, args=(True,), full_output=True, disp=False
        )[:2]
        ...
        if fposs:
            ...
            opt_x = fx.flatten()
            opt_x, sig2 = fmin(
                self.calc_deviat, opt_x, full_output=True, disp=False
            )[:2]
...
class Cryplots:
    ...
    def _plot(cls, xrd, fname):
        x_label = {"theta": "$\\theta$", "2theta": "$2\\theta$", "q": "q"}.get(
            xrd.x_units, _("Unknown")
        )
        ...
        for x0, h, w, s in cryb.reshape(len(cryb) // 4, 4):
            ...
            if xrd.x_units == "theta":
                x = np.arcsin(x) * 180.0 / np.pi
            elif xrd.x_units == "2theta":
                x = np.arcsin(x) * 360.0 / np.pi
            plots.append({"x1": x, "y1": y, "color": "crp_refl"})
        plots.append(
            {
                "x1": xrd.x_data,
                "y1": cls._calc_shape(xrd, shfunc),
                "color": "crp_srefl",
            }
        )
        plt["plots"] = plots
