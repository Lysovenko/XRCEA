...
def d_hkl_orhomb(a, b, c, hkl):
    ...
    h, k, el = hkl
    d2 = 1.0 / a**2 * h**2 + 1.0 / b**2 * k**2 + 1.0 / c**2 * el**2
    return sqrt(1.0 / d2)
...
def d_hkl_tetra(a, c, hkl):
    ...
    h, k, el = hkl
    d2 = 1.0 / a**2 * (h**2 + k**2) + 1.0 / c**2 * el**2
    return sqrt(1.0 / d2)
...
def d_hkl_rhombohedral(a, alp, hkl):
    ...
    d2 = (
        ...
        * (
            (h**2 + k**2 + el**2)
            - (1.0 - tan(alp / 2.0) ** 2) * (h * k + k * el + el * h)
        )
        / (1.0 + cos(alp) - 2.0 * cos(alp) ** 2)
    )
    return sqrt(1.0 / d2)
...
class FitIndices:
    def __init__(self, crystal_system, max_ind):
        ...
        hkl = array(
            list(product(*((tuple(range(max_ind + 1)),) * 3)))[1:]
        ).transpose()
...
class CellParams:
    ...
    def __init__(self, xrd):
        ...
        for name in indset:
            ...
            try:
                res[name] = (
                    CALCULATORS[indset[name]["cell"]](get_dhkl(ipd, inds)),
                    indset[name]["cell"],
                )
