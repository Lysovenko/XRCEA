...
class DMenu:
    ...
    def get_items(self, path, other=None):
        ...
        rlist = [
            ((t_con[i].priority + o_con[i].priority) / 2, i, t_con[i])
            for i in isec
        ]
        rlist += [
            (t_con[i].priority, i, t_con[i]) for i in uniq.intersection(t_con)
        ]
        rlist += [
            (o_con[i].priority, i, o_con[i]) for i in uniq.intersection(o_con)
        ]
        rlist.sort()
    ...
    def get_container(self, path, default=None):
        ...
        for i, name in enumerate(path):
            try:
                ...
                if default is None:
                    rcont = {}
                    self.append_item(path[:i], path[i], rcont)
                else:
                    return default
        return rcont
    ...
    def insert_item(
        ...
        if name in cont:
            if isinstance(cont[name].function, dict) and isinstance(
                function, dict
            ):
                return
            raise RuntimeError(
                "only submenu can be registred twice %s/%s"
                % ("/".join(path), name)
            )
    ...
    def append_item(
        self, path, name, function, shortcut=None, description=None, icon=None
    ):
        ...
        if cont:
            ...
            priority = 1
        self.insert_item(
            cont, priority, name, function, shortcut, description, icon
        )

    def insert_item_after(
        self, path, name, function, shortcut, description=None, icon=None
    ):
